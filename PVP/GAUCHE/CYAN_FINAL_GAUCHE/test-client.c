...
Point Cyan_behavior(Dog *cyan, double xmax, double ymax){
    Point objective, point1, point2;
    Node target1, target2;

    point1.x = xmax - 1.05*CYAN_RADIUS;
    point1.y = ennemy_sheepfold_center.y - sheepfold_radius;

    point2.x = xmax - 1.05*CYAN_RADIUS;
...
int receive_packet(struct lws *wsi, unsigned char * buf){
    ...
    switch(typeMsg){
        ...
        case 32 :
            myId = getMyId(buf);
            cyan_node = create_node(myId, create_point(0, 0), "yellow1");
        ...
        case 64:
            if(myId == 0){
                ...
                p = Cyan_behavior(&cyan_dog, xMax, yMax);
