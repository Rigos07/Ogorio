...
#IA des brebis
#IA des chiens rameneurs par couleur
#Yellow suivent le circuit
#IA de scout
#liste de moutons par Blue
#Lucas GHENIA
...
def dog_scout_scan(dog):
    for i in range(0, len(SHEEPS_LIST)):
        sheep = SHEEPS_LIST[i]
        dx = sheep.x - dog.x
        dy = sheep.y - dog.y
        if( abs(dx) < dog.sightx and abs(dy) < dog.sighty ): #if sheep in sight
            distance_sheep_to_sheepfold = sqrt( (sheep.x - sheepfold_center[0])**2 + (sheep.y - sheepfold_center[1])**2)
            if( distance_sheep_to_sheepfold > sheepfold_radius):#if sheep not in sheepfold
                if( not(sheep in dog.sheeps) ): #if not already in list
                    dog.sheeps.append(sheep)
